...
if(!empty($name) && !empty($Email) && !empty($pass) && !empty($Institute) && !empty($gender))
...
#------------
$q = " select * from students where name = '$name' && password = '$pass' && email='$Email' ";
...
window.location.href='www.learnbyexample.in/registration/index.html';
	...
	$qy = " insert into students(name,password,email,gender,institute) values ('$name','$pass','$Email','$gender','$Institute')";
    mysqli_query($con,$qy);
